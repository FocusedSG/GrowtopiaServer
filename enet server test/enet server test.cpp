...
string hashPasswordverify(string passwordverify) {
	...
	assert(ret == 0);
	ret = bcrypt_hashpw(passwordverify.c_str(), salt, hash);
	assert(ret == 0);
	return hash;
}
...
public:
	...
	static int playerRegister(string username, string password, string email, string passwordverify);
};
...
int PlayerDB::playerRegister(string username, string password, string email, string passwordverify) {
	...
	j["passwordverify"] = hashPasswordverify(passwordverify);
	j["email"] = email;
	j["adminLevel"] = 0;
	o << j << std::endl;
	return 1;
				...
				if (cch.find("action|dialog_return") == 0)
				{
					...
					if (isRegisterDialog) {
						...
						if (regState == 1) {
							...
							string passwordverify = "passwordverify";
							string email = "email";
							ENetPacket * packet2 = enet_packet_create(p2.data,
								p2.len,
								ENET_PACKET_FLAG_RELIABLE);
